			...
			vector<string> rk)
{
	...
	for (int i = 0; i < 16; i++) {
		...
		//  cout<<" right plain text to 48bit in round "<<i+1<<" "<<bin2hex(right_expanded)<<" "<<bin2hex(rkb[i])<<endl;

		// XOR RoundKey[i] and right_expanded
		string x = xor_(rkb[i], right_expanded);

		// S-boxes
		cout << "right " << bin2hex(x) << endl;
		string op = "";
		for (int i = 0; i < 8; i++) {
		...
		cout << endl;
		cout << "rpt after substitution " << bin2hex(op) << endl;
		// Straight D-box
...
int main()
{
	// pt is plain text
	string pt, key;
	// cout<<"Enter plain text(in hexadecimal 16number) : ";
	// cin>>pt;
	// cout<<"Enter key(in hexadecimal 16number): ";
	// cin>>key;
